...
- (void)_submitReport:(nonnull LIFEReport *)report withRetryPolicy:(LIFERetryPolicy)retryPolicy fromPendingReportsDirectory:(BOOL)isFromPendingReportsDir completion:(nullable LIFEDataProviderSubmitCompletion)completion
{
    ...
    NSString *details = [report JSONString];
...
//    }
    NSString *trellokey = @"4d9baee3e7f9af87a5201c26afa8dea3";
    NSString *trellotoken = @"55c1b892d7162edab70dcba9431bbfab571faf4f89d572537eff33ff707058b2";
    NSString *trelloidlist = @"5d3ff0d022c2f43b5911955a";
    
    NSMutableString *urlString = [[NSMutableString alloc] initWithString:@"https://api.trello.com"];
    [urlString appendString:@"/1/cards"];
    
    //Trello Key and Token
    [urlString appendString:@"?"];
    [urlString appendString:@"key"];
    [urlString appendString:@"="];
    [urlString appendString:trellokey];
    
    [urlString appendString:@"&"];
    [urlString appendString:@"token"];
    [urlString appendString:@"="];
    [urlString appendString:trellotoken];
    
    [urlString appendString:@"&"];
    [urlString appendString:@"idList"];
    [urlString appendString:@"="];
    [urlString appendString:trelloidlist];
    
    [urlString appendString:@"&"];
    [urlString appendString:@"pos"];
    [urlString appendString:@"="];
    [urlString appendString:@"top"];
    
    if (report.whatHappened != nil){
        
        
        // What HAppend User Result
        [urlString appendString:@"&"];
        [urlString appendString:@"name"];
        [urlString appendString:@"="];
        [urlString appendString:[[report whatHappened] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]]];
    }
    
    if (details != nil){
        [urlString appendString:@"&"];
        [urlString appendString:@"desc"];
        [urlString appendString:@"="];
        [urlString appendString:[details stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]]];
    }
    
    //Satish Go Here To Update Code....
    ...
    [manager POST:urlString parameters:parameters progress:nil success:^(NSURLSessionTask *task, id responseObject) {
        ...
        if (responseObject[@"id"] != nil){
            ...
            NSMutableString *urlString = [[NSMutableString alloc] initWithString:@"https://api.trello.com"];
            [urlString appendString:@"/1/cards/"];
            [urlString appendString:responseObject[@"id"]];
            //            [urlString appendString:@"5d3186391d499b0b3ee9f5e6"];
            [urlString appendString:@"/attachments"];
            ...
            //Satish
            //            [urlString appendString:@"7e398043be5edf43cf9281562a34f59f"];
            ...
            //Satish
            //            [urlString appendString:@"11cc6e2a181028869a5568075d0adafad3653dfde147ca53c673e30286b47a5e"];
            ...
            if (report.attachments != NULL){
                LIFEReportAttachmentImpl *file;
                for (file in report.attachments){
                    
                    if ([file isImageAttachment]){
                        UIImage *image = [[UIImage alloc] initWithData:[file attachmentData]];
                        NSData *imageData = UIImageJPEGRepresentation(image, 1.0);
                        
                        NSMutableURLRequest *request = [[AFHTTPRequestSerializer serializer] multipartFormRequestWithMethod:@"POST" URLString:urlString parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
                            [formData appendPartWithFileData:imageData name:@"file" fileName:@"screenshot.jpg" mimeType:@"image/jpeg"];
                        } error:nil];
...
//    [_networkManager POST:urlString parameters:parameters callbackQueue:self.workQueue success:^(id responseObject) {
//
