...
androidRequestMultiplePermissoin = (permission) => {
    return Promise.resolve(PermissionsAndroid.requestMultiple(permission)
        .then(response => {
            return response;
        }));
};
...
androidRequestPermission = (permission) => {
    switch (permission) {
        case Constants.Permissions.PHOTO:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
                Constants.Permissions.STORAGE));
        case Constants.Permissions.CAMERA:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.CAMERA,
                Constants.Permissions.CAMERA));
        case Constants.Permissions.LOCATION:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.ACCESS_COARSE_LOCATION,
                Constants.Permissions.LOCATION));
        case Constants.Permissions.READ_EXTERNAL_STORAGE:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.READ_EXTERNAL_STORAGE,
                Constants.Permissions.STORAGE));
        case Constants.Permissions.WRITE_EXTERNAL_STORAGE:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.WRITE_EXTERNAL_STORAGE,
                Constants.Permissions.STORAGE));
        case Constants.Permissions.CONTACTS:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.READ_CONTACTS,
                Constants.Permissions.CONTACTS));
        case Constants.Permissions.PHONE_CALL:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.CALL_PHONE,
                Constants.Permissions.PHONE_CALL));
        case Constants.Permissions.READ_SMS:
            return Promise.resolve(androidCheckPermissoin(PermissionsAndroid.PERMISSIONS.READ_SMS,
        ...
        [
            {
                ...
                onPress: Platform.OS === 'ios' ? Linking.openURL('app-settings:') : OpenSettings.openSettings
            },
            {
...
requestNotificationPermission = () => {
    return Promise.resolve(requestPermission(Constants.Permissions.PUSH_NOTIFICATION));
};
